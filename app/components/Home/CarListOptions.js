import { CarListData } from '@/public/utils/CarListData';
...
function CarListOptions({distance}) {
  ...
  return (
    <div className='mt-5 p-5 overflow-auto h-[250px]'>
       ...
       bottom-5 bg-black p-3 shadow-xl rounded-lg
       w-full md:w-[30%] border-[1px] items-center'>
        <h2>Make Payment For</h2>
        <button className='
        p-3 bg-white text-black rounded-lg
        text-center'
        onClick={()=>router.push('/payment?amount='+(selectedCar.amount*distance*80).toFixed(2))}
